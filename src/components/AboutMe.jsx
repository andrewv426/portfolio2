...
export default function AboutMe() {
  ...
  // Track scroll progress through this entire 300vh section
  const { scrollYProgress } = useScroll({
    target: sectionRef,
    offset: ["start start", "end end"]
  })

  // 250vh container → 150vh of tracked scroll.
  // Fade in 0→0.4 (60vh), hold solid 0.4→0.6 (30vh), fade out 0.6→1 (60vh).
  const contentOpacity = useTransform(scrollYProgress, [0, 0.4, 0.6, 1], [0, 1, 1, 0])
  const contentScale = useTransform(scrollYProgress, [0, 0.4, 0.6, 1], [0.95, 1, 1, 0.95])

  return (
    <section
      id="about"
      ref={sectionRef}
      // Reduced to 250vh for snappier pacing.
      className="relative h-[250vh] w-full bg-transparent z-10"
    >
      {/* Sticky Container */}
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-[864px] mx-auto px-4 md:px-8 py-20 flex flex-col md:flex-row items-center md:justify-between gap-12 md:gap-8 font-['Karla'] text-[24px] text-[#fef9ed] leading-normal mt-10 md:mt-0 drop-shadow-md"
        >
